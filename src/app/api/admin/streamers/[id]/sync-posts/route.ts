...
import { syncPostsSchema } from "@/lib/validation/posts";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";
/** Paging plus one insights call per post takes longer than a default lambda. */
export const maxDuration = 300;

/**
 * POST /api/admin/streamers/{id}/sync-posts
 *
 * Runs synchronisation inline and returns the outcome. Admin-only: it spends
 * Meta API quota and writes metrics.
 *
 * A run that partially succeeds returns 200 with `status: "partial"` rather
 * than an error status — the posts that were collected are real, and reporting
 * the whole run as a failure would hide them.
 */
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  // The body is optional — an empty POST means "sync with defaults".
  let body: unknown = {};
  try {
